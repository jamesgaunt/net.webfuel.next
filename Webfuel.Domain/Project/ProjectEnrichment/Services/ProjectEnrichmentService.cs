...
using Webfuel.Client;
using Webfuel.Domain;
using Webfuel.Domain.StaticData;
using Webfuel.Heartbeats;

namespace Webfuel.Domain
{
    public interface IProjectEnrichmentService: IHeartbeat
    {
    ...
    [Service(typeof(IProjectEnrichmentService), typeof(IHeartbeat))]
    internal class ProjectEnrichmentService: IProjectEnrichmentService
    {
        ...
        public async Task<HeartbeatResult?> ExecuteHeartbeat(HeartbeatRecord record)
        {
            ...
            if (project == null)
            {
                ...
                {
                    Message = "No projects requiring heartbeat",
                    SleepFor = TimeSpan.FromHours(1),
                };
            }
            ...
            try
            {
                ...
                return new HeartbeatResult
                {
                    Message = "Executed heartbeat for project " + project.PrefixedNumber,
                    Processed = 1,
                    SleepFor = TimeSpan.FromMinutes(2)
                };
            }
            finally
            {
