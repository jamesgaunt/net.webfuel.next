...
public class QueryProject : Query, IRequest<QueryResult<Project>>
{
    public Query ApplyCustomFilters()
    {
        ...
        if (OpenSupportTeamId.HasValue)
            this.SQL($"EXISTS (SELECT Id FROM [ProjectSupport] AS ps WHERE ps.[ProjectSupportGroupId] = e.[ProjectSupportGroupId] AND ps.[SupportRequestedTeamId] = '{OpenSupportTeamId.Value}' AND ps.[SupportRequestedCompletedAt] IS NULL)");
