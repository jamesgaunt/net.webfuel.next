...
internal class ReportAsyncMapping<TEntity, TMap> : IReportMapping<TEntity, TMap>
    where TEntity : class
    where TMap : class, IReportMap<TEntity>
{
    ...
    [JsonIgnore]
    public required Func<object, IServiceProvider, TMap, Task<Guid?>> Accessor { get; init; }
    ...
    public async Task<List<object>> MapContextsToEntities(List<object> contexts, ReportBuilder builder)
    {
        ...
        {
            var id = await Accessor(context, builder.ServiceProvider, map);
            if (id == null)
                continue;
