...
import { QueryFileStorageEntry, QueryResult, FileStorageEntry, UpdateFileStorageEntry } from './api.types';

@Injectable()
export class FileStorageEntryApi implements IDataSource<FileStorageEntry, QueryFileStorageEntry, any, UpdateFileStorageEntry> {
    ...
    public update (body: UpdateFileStorageEntry, options?: ApiOptions): Observable<FileStorageEntry> {
        return this.apiService.request<UpdateFileStorageEntry, FileStorageEntry>("POST", "api/file-storage-entry/update", body, options).pipe(tap(_ => this.changed.emit()));
    }
